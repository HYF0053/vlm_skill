...
# ── Single-pass Top-K search with time-decay ─────────────────────────────────────────

def fetch_point_by_id(point_id: str, collection: str, cfg: dict) -> dict | None:
    # Not heavily needed for single turn search without 'adjacent' chunks, but kept for util usage
...
def _apply_time_decay(hits: list[dict], time_weight: float) -> list[dict]:
    ...
    recency = (ts_unix - t_min) / (t_max - t_min), normalised to [0, 1].
    newest turn → recency=1.0, oldest → recency=0.0.
    """
    ...
    ts_vals = [h["payload"].get("ts_unix", h["payload"].get("indexed_at_unix", 0)) for h in hits]
    if not ts_vals:
        return hits
    ...
    for h in hits:
        sem = h.get("score", 0.0)
        ts  = h["payload"].get("ts_unix", h["payload"].get("indexed_at_unix", t_min))
...
def search_turn_level_topk(
    ...
    """
    Memory V3 Search:
    Retrieve Top-K individual turns semantically.
    Filter by type = "turn".
    Apply time-decay blend score; sort descending.
    """
    ...
    # Time-decay blend + sort 
    turn_hits = _apply_time_decay(turn_hits, time_weight)
    turn_hits.sort(key=lambda x: x.get("blended_score", x.get("score", 0)), reverse=True)
    return turn_hits[:limit]
...
def print_hit(i: int, hit: dict, show_full: bool = False) -> None:
    ...
    display_time = (
        p.get("dt") or p.get("chunk_turn_start_ts") or p.get("indexed_at_iso") or p.get("iso_time") or ""
    )[:19]

    print(f"[{i}] {kind}  score={score_str}  time={display_time}")

    if ptype == "turn":
        print(f"    session={p.get('session_key')}")
    ...
    elif is_conv or ptype in ("chunk", "session_summary"):
        # Legacy formatting for old V1/V2 archives
        if ptype == "session_summary":
            ...
            print(f"    session={p.get('session_key')}  turns={turn_range}")
            print(f"    summary: {p.get('summary', '')}")
...
def main():
    ...
    parser.add_argument("--min-score",      type=float, default=0.0,
                        help="Minimum final score (rerank or blend) to keep the result")
    ...
    if args.type in ("all", "conv"):
        # V3 relies entirely on simple turn-level search
        tp_hits = search_turn_level_topk(
        ...
        if args.mode == "legacy" or not tp_hits:
            ...
            for col in conv_cols:
                # specifically search for legacy chunks
                hits = search_collection(col, vector, fetch_per_col, conv_filter, cfg)
                legacy_hits = [h for h in hits if h.get("payload", {}).get("type") in ("chunk", "session_summary")]
    ...
    main_hits = rerank(query_str, main_hits, cfg)

    # Threshold filtering
    if args.min_score > 0:
        filtered_hits = []
        for h in main_hits:
            score_to_check = h.get("rerank_score", h.get("blended_score", h.get("score", 0)))
            if score_to_check >= args.min_score:
                filtered_hits.append(h)
        main_hits = filtered_hits

    main_hits = main_hits[:args.limit]
