...
Agent 在回答問題時，會遵循以下優先順序以確保資訊的準確性與即時性：
1. **當前上下文 (短期)**：如果資訊就在目前的對話窗口內，Agent 直接回答。
2. **RAG / 記憶 (長期)**：涉及「上次決定」、「過去討論」、「使用者偏好」或「專案規格」時觸發。
3. **MCP 伺服器 (即時/外部資料)**：需要讀取外部服務（如 GitHub PR、Slack 訊息）或執行特定動作時觸發。
4. **Web Search (外部聯網知識)**：當內部記憶找不到結果，或是問題涉及通用的最新外部技術知識時使用。
