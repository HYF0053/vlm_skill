...
def find_config_file() -> Optional[Path]:
    ...
    # Priority 4: config/mcp.json (Project MCP config)
    # Resolve from script location: scripts -> mcp_client -> skills -> project_root
    script_dir = Path(__file__).parent
    project_root = script_dir.parent.parent.parent
    project_config = project_root / "config" / "mcp.json"
...
async def create_session(config: dict):
    ...
    if transport == "fastmcp":
        # FastMCP client (StreamableHttp or SSE), optional Bearer auth
        ...
        if "/mcp/sse" in url or "/sse" in url:
            transport_obj = SSETransport(url, headers=headers if headers else None)
        else:
            transport_obj = StreamableHttpTransport(url, headers=headers if headers else None)
